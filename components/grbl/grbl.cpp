...
void Grbl::parse_grbl_response_(const std::string& line) {
    ...
    if (line[0] == '<' && line[line.size() - 2] == '>') {
        ...
        int error = atoi(line.c_str() + 6);
        ESP_LOGW(TAG, "Received GRBL error: %d", error);
        this->error_callback_.call(error);

    } else if (line.compare(0, 6, "ALARM:") == 0) {
        // GRBL alarm message, e.g. "ALARM:1"
        int alarm = atoi(line.c_str() + 6);
        ESP_LOGW(TAG, "Received GRBL alarm: %d", alarm);
        this->status_.state = State::STATE_ALARM;
        this->alarm_callback_.call(alarm);
        this->state_text_sensor_->publish_state("alarm");

    } else if (line.compare(0, 25, "[MSG:'$H'|'$X' to unlock]") == 0) {
        ESP_LOGW(TAG, "Initial GRBL alarm state");
        this->alarm_callback_.call(0);
        ...
        for (auto* listener : this->settings_listeners_) {
